...
public class InstallServer extends AsyncTask<Context, Void, Void> {
    ...
    public static String getPublicWritableFolder(Context context){
    ...
    void installSqliteFiles(Context context){
        ...
        String basicPath = getPublicWritableFolder(context);

        File externalDbDir = new File( basicPath  +
                File.separator + "www" + File.separator + "db");

        externalDbDir.mkdirs();

        File tmpFolder = new File( basicPath  +
                File.separator + "files" + File.separator + "tmp");
